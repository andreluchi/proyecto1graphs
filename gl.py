...
class Renderer(object):
    ...
    def glTriangle_bc(self, A, B, C, texCoords = (), normals = (), verts = (),  color = None):
        ...
        triangleNormal = al.cross(np.subtract(verts[1],verts[0]), np.subtract(verts[2],verts[0]))
    ...
    def glLookAt(self, eye, camPosition = V3(0,0,0)):
        ...
        forward = forward / al.norm(forward)

        right = al.cross(V3(0,1,0), forward)
        right = right / al.norm(right)

        up = al.cross(forward, right)
        up = up / al.norm(up)
